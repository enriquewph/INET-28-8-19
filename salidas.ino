#include "header.h"

void control_HUMEDAD()
{
    if (HUMEDAD <= HUMEDAD_TRIGGER && HUMEDAD_ESTADO == 0 && HUMEDAD < 100 && !EV_ERROR_SENSOR_HUMEDAD_ACTIVADO)
    {
        HUMEDAD_ESTADO = MODO_REGADO_ENCENDIDO;
        lcd_createEvent(EV_HUMEDAD_BAJA, EV_INFORMACION);
    }
    else if (EV_ERROR_SENSOR_HUMEDAD_ACTIVADO)
        HUMEDAD_ESTADO = MODO_REGADO_APAGADO;
    if (HUMEDAD >= HUMEDAD_RELEASE && HUMEDAD_ESTADO == 1 && HUMEDAD < 100)
    {
        HUMEDAD_ESTADO = MODO_REGADO_APAGADO;
        lcd_createEvent(EV_HUMEDAD_ALTA, EV_INFORMACION);
    }
    FUNCIONAMIENTO_REGADO = HUMEDAD_ESTADO;
}

void control_TEMP()
{
    if (TEMPERATURA <= TEMPERATURA_BAJA_TRIGGER && TEMPERATURA_BAJA_ESTADO == 0 && !EV_ERROR_SENSOR_TEMPERATURA_ACTIVADO)
    {
        TEMPERATURA_BAJA_ESTADO = 1;
        FUNCIONAMIENTO_TEMP = MODO_TEMP_CALENTANDO;
        lcd_createEvent(EV_TEMP_BAJA, EV_INFORMACION);
    }
    else if (EV_ERROR_SENSOR_TEMPERATURA_ACTIVADO)
        FUNCIONAMIENTO_TEMP = MODO_TEMP_STANDBY;
    if (TEMPERATURA >= TEMPERATURA_RELEASE && TEMPERATURA_BAJA_ESTADO == 1)
    {

        TEMPERATURA_BAJA_ESTADO = 0;
        FUNCIONAMIENTO_TEMP = MODO_TEMP_STANDBY;
        lcd_createEvent(EV_TEMP_ESTABILIZADA, EV_INFORMACION);
    }

    if (TEMPERATURA >= TEMPERATURA_ALTA_TRIGGER && TEMPERATURA_ALTA_ESTADO == 0 && !EV_ERROR_SENSOR_TEMPERATURA_ACTIVADO)
    {
        TEMPERATURA_ALTA_ESTADO = 1;
        FUNCIONAMIENTO_TEMP = MODO_TEMP_ENFRIANDO;
        lcd_createEvent(EV_TEMP_ALTA, EV_INFORMACION);
    }
    else if (EV_ERROR_SENSOR_TEMPERATURA_ACTIVADO)
        FUNCIONAMIENTO_TEMP = MODO_TEMP_STANDBY;
    if (TEMPERATURA <= TEMPERATURA_RELEASE && TEMPERATURA_ALTA_ESTADO == 1)
    {
        TEMPERATURA_ALTA_ESTADO = 0;
        FUNCIONAMIENTO_TEMP = MODO_TEMP_STANDBY;
        lcd_createEvent(EV_TEMP_ESTABILIZADA, EV_INFORMACION);
    }
}

void TEMPERATURA_UPDATE_RELEASE()
{
    TEMPERATURA_RELEASE = (TEMPERATURA_ALTA_TRIGGER + TEMPERATURA_BAJA_TRIGGER) / 2;
    Serial.println(TEMPERATURA_RELEASE);
}

void setearSalidas()
{
    digitalWrite(PIN_RELAY_MOTOR, SALIDA_EXTRACTOR);
    digitalWrite(PIN_RELAY_CALEFACTOR, SALIDA_CALEFACTOR);
    digitalWrite(PIN_RELAY_RIEGO, SALIDA_REGADOR);
    analogWrite(PIN_DIMMER_LUZ, pwmmap(POTENCIA_LUCES, 0, 100, 0, 255));
}

uint8_t pwmmap(uint16_t x, uint16_t in_min, uint16_t in_max, uint8_t out_min, uint8_t out_max)
{
    return (x - in_min) * (out_max - out_min) / (in_max - in_min) + out_min;
}